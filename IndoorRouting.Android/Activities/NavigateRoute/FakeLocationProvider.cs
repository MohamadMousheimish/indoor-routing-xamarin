...
namespace IndoorRouting.Android.Activities.NavigateRoute
{
    public class FakeLocationProvider: LocationDataSource
    {
        ...
        public FakeLocationProvider(Polyline route)
        {
            ...
            var densifieRoute = (Polyline)GeometryEngine.DensifyGeodetic(route, 25.0, LinearUnits.Meters);

            // Get all of the points from the densified polyline
            _gpsPoints = new List<MapPoint>(densifieRoute.Parts[0].Points);
        ...
        private void TimerElapsed(object sender, ElapsedEventArgs e)
        {
            ...
            if(_nodeIndex > 0)
            {
                ...
                var distance = GeometryEngine.DistanceGeodetic(_gpsPoints[_nodeIndex], _gpsPoints[_nodeIndex - 1], LinearUnits.Meters, AngularUnits.Degrees, GeodeticCurveType.Geodesic);
